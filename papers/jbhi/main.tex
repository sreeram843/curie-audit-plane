...
Optional Synthea slices are accepted only from roots listed in an approved
manifest~\cite{walonoski2018synthea}.
...
Software-quality checks at the time of this draft were $135$ Python tests,
$94.44\%$ statement coverage, and a clean Ruff pass; those are not clinical
performance.
...
Mean plane latency is $12.0$\,ms versus $6.7$\,ms for the identical
unrecorded workflow (paired latency ratio $0.80$, $95\%$ interval
$[0.72, 0.88]$, $n=3$ after one warmup). Storage ratio versus that workflow is
...
application. Verification of the clean transaction is $0.94$\,ms.
...
coverage, not a population sample. Console reconstruction is
...
and CLI parameters must be recorded by the operator of any local dump; this
repository stores an approved-root manifest and license, not patient dumps.
A DOI for the public archive is not assigned in this draft.
